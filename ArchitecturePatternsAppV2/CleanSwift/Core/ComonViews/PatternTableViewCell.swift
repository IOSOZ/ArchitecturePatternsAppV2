...
final class PatternTableViewCell: UITableViewCell {
    ...
    private var object: PatternViewModel!
    
    // MARK: - PatternTableViewCellDelegate
    
    weak var delegate: PatternTableViewCellDelegate?
    ...
    // MARK: - UI SetUP
    func configure(with viewModel: PatternViewModel, isFirstCell: Bool) {
        self.object = viewModel
        ...
        viewCounterLabel.getAttributedStringWith(
            title: "Посмотрено раз: \(viewModel.viewCounter)",
        ...
        patternImage.image = viewModel.image
        
        favoriteButton.setImage(
            viewModel.isFavorite == false ? UIImage(named: "notLike") : UIImage(named: "like"),
            for: .normal
        )
